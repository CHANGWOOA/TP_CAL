...
const views = {
  ...
  detail: async (req, res) => {
    ...
    res.render("board/board_detail", {
      data: data,
      replyList: repResult.rows,
      username: req.session.username,
    }); //data를 ejs로 보낸다->ejs 사용하지 않고 data[0].P_ID 형식으로 바로 정보 받을 수 있음
  ...
  search: async (req, res) => {
    ...
    const result = await ser.boardRead.search(
      req.body.searchType,
      req.body.searchKey
    );
...
const process = {
  ...
  modify: (req, res) => {
    //게시글 수정 - 없어서 terminal이 난리라 만들어두었습니다.
